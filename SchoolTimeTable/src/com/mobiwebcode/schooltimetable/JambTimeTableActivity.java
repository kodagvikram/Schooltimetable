...
public class JambTimeTableActivity extends Activity {
	ListView jambListView;
	...
	NodeList nl;
	Document doc;
	String JAMB_TIMETABLE_URL = "http://millionairesorg.com/schooltimetable/timetabledetail.php?timetabletype="
			+ "jamb" + "&subject=" + Selectsubjectjamb.selectedsubject;
	ArrayList<TimetableVO> jambTimetableList = new ArrayList<TimetableVO>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.jambtimetableactivity);
		Save = (Button) findViewById(R.id.btnsave);
		jambListView = (ListView) findViewById(R.id.jambListView);
		...
		if (file.exists() == true && isSave == false) {
			...
			new myTask_jamb_Timetable_call().execute("");
			isSave = false;
		}
			...
			public void onClick(View v) {
				...
				if (Save.getText().toString().equals("Save")) {
					try {
						...
						myFolder.mkdirs();
						File myFile = new File("/sdcard/.skubag/jamb.txt");
						...
						final AlertDialog alertDialog = new AlertDialog.Builder(
								JambTimeTableActivity.this).create();
					...
					Intent i = new Intent(JambTimeTableActivity.this,
	...
	class myTask_jamb_Timetable_call extends AsyncTask<String, Void, String> {
		@Override
		...
		protected String doInBackground(String... aurl) {
			try {
				xml = parser.getXmlFromUrl(JAMB_TIMETABLE_URL);
	...
	public class CustomList extends BaseAdapter {
		...
		public View getView(final int position, final View view,
			...
			View rowView = inflater.inflate(R.layout.timetable_listview, null,
					true);
			TimetableVO ttvo = jambTimetableList.get(position);
		...
		if (doc != null) {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (ttvo.country.toString().equalsIgnoreCase("all")
					...
					jambTimetableList.add(ttvo);
				}
			}
		}
		CustomList adapter = new CustomList(JambTimeTableActivity.this,
				jambTimetableList);
		jambListView.setAdapter(adapter);
