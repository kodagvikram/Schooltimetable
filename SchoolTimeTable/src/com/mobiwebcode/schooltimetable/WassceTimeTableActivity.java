...
public class WassceTimeTableActivity extends Activity {
	ListView wassceListView;
	...
	NodeList nl;
	Document doc;
	String WASSCE_TIMETABLE_URL = "http://millionairesorg.com/schooltimetable/timetabledetail.php?timetabletype="
			+ "wassce" + "&subject=" + SelectWschesubject.selectedsubject;
	ArrayList<TimetableVO> wassceTimetableList = new ArrayList<TimetableVO>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.wasscetimetablelist);
		Save = (Button) findViewById(R.id.btnsave);
		wassceListView = (ListView) findViewById(R.id.wassceListView);
		if (isSave == true) {
			Save.setText("Save");
		...
		if (file.exists() == true && isSave==false) {
			...
			new myTask_WASSCE_Timetable_call().execute("");
			isSave=false;
		}
			...
			public void onClick(View v) {
				...
				if (Save.getText().toString().equals("Save")) {
					try {
						...
						myFolder.mkdirs();
						File myFile = new File("/sdcard/.skubag/wassce.txt");
						...
						final AlertDialog alertDialog = new AlertDialog.Builder(
								WassceTimeTableActivity.this).create();
						alertDialog.setIcon(R.drawable.ic_launcher);
						alertDialog.setTitle("SKUBAG");
						alertDialog.setMessage("Timetable Saved Successfully!!!");
					...
					Intent i = new Intent(WassceTimeTableActivity.this,
	...
	//
	// StringBuffer output = new StringBuffer();
	// String fpath = "/sdcard/mysdfile.txt";
	//
	...
	class myTask_WASSCE_Timetable_call extends AsyncTask<String, Void, String> {
		@Override
		...
		protected String doInBackground(String... aurl) {
			try {
				xml = parser.getXmlFromUrl(WASSCE_TIMETABLE_URL);
	...
	public class CustomList extends BaseAdapter {
		...
		public View getView(final int position, final View view,
			...
			View rowView = inflater.inflate(R.layout.timetable_listview, null,
					true);
			TimetableVO ttvo = wassceTimetableList.get(position);
		...
		if (doc != null) {
			...
			for (int i = 0; i < nl.getLength(); i++) {
				...
				if (ttvo.country.toString().equalsIgnoreCase("all")
					...
					wassceTimetableList.add(ttvo);
				}

			}
		}
		CustomList adapter = new CustomList(WassceTimeTableActivity.this,
				wassceTimetableList);
		wassceListView.setAdapter(adapter);
